...
Extraction happens **without running any lifecycle script**, with strict path handling (no escapes, no symlinks) and bounded file counts, sizes, and total bytes. See **[Decisions & trust](/docs/concepts/decisions-and-trust/)** for the trust levels.
...
- **React Native native surface** — iOS/Android directories, `.podspec`, `build.gradle`, `CMakeLists.txt`, prebuilt binaries and frameworks, autolinking config, and dangerous Android permissions. See **[React Native hardening](/docs/concepts/react-native/)**.
...
Read on: **[Decisions & trust](/docs/concepts/decisions-and-trust/)** · **[Approvals & policy](/docs/concepts/approvals-and-policy/)**.
