...
describe("policy formats", () => {
  it("loads bye.policy.yaml", async () => {
    const cwd = await scratch();
    await writeFile(
      path.join(cwd, "bye.policy.yaml"),
      "dependencyPolicy:\n  minPackageAgeDays: 14\n",
    );
    const loaded = await loadPolicy(cwd);
    expect(loaded?.policy.dependencyPolicy.minPackageAgeDays).toBe(14);
    expect(path.basename(loaded!.file)).toBe("bye.policy.yaml");
  });

  it("loads bye.policy.json", async () => {
    const cwd = await scratch();
    await writeFile(
      path.join(cwd, "bye.policy.json"),
  ...
  it("loads bye.policy.js (default export)", async () => {
    const cwd = await scratch();
    await writeFile(
      path.join(cwd, "bye.policy.js"),
  ...
  it("loads bye.policy.ts with type annotations", async () => {
    const cwd = await scratch();
    await writeFile(
      path.join(cwd, "bye.policy.ts"),
      [
  ...
  it("prefers ts over yaml over json when multiple exist", async () => {
    ...
    await writeFile(path.join(cwd, "bye.policy.yaml"), "dependencyPolicy:\n  minPackageAgeDays: 2\n");
    await writeFile(
      path.join(cwd, "bye.policy.ts"),
...
describe("policy init formats", () => {
  it.each(["yaml", "json", "js", "ts"] as const)("scaffolds a loadable %s policy", async (format) => {
    ...
    expect(file).toBe(path.join(cwd, `bye.policy.${format}`));
    const loaded = await loadPolicy(cwd);
  ...
  it("ts template uses a type-only import (loads without bye installed)", async () => {
    const cwd = await scratch();
    const file = await initPolicy(cwd, "ts");
    const content = await readFile(file!, "utf8");
    expect(content).toContain('import type { PolicyFile } from "bye"');
...
describe("approvals formats", () => {
  it("reads yaml approvals", async () => {
    const cwd = await scratch();
    await mkdir(path.join(cwd, ".bye"));
    await writeFile(
      path.join(cwd, ".bye", "approvals.yaml"),
  ...
  it("reads ts approvals", async () => {
    ...
    await mkdir(path.join(cwd, ".bye"));
    await writeFile(
      path.join(cwd, ".bye", "approvals.ts"),
  ...
  it("recording never touches hand-curated sources", async () => {
    ...
    await writeFile(path.join(cwd, ".bye", "approvals.ts"), tsSource);
    await recordApproval("c", "2.0.0", "normal", cwd);
    expect(await readFile(path.join(cwd, ".bye", "approvals.ts"), "utf8")).toBe(tsSource);
    const json = JSON.parse(await readFile(path.join(cwd, ".bye", "approvals.json"), "utf8"));
  ...
  it("ignores a broken source instead of crashing", async () => {
    const cwd = await scratch();
    await mkdir(path.join(cwd, ".bye"));
    await writeFile(path.join(cwd, ".bye", "approvals.js"), "export default {{{ nope\n");
    await writeFile(
      path.join(cwd, ".bye", "approvals.json"),
