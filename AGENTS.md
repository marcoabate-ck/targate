# Dependency installs — use bye

`bye` (Before You Execute) is a pre-install security gate for npm packages. Installing a package runs its install-time lifecycle scripts on this machine, so every new dependency must be analyzed by `bye` **before** it is installed.
