...
export function renderSignalLines(signals: Signals): string[] {
  ...
  lines.push(
    "  " + check(signals.repositoryMissing, "repository metadata present", "repository metadata missing"),
  );
  lines.push(
    "  " +
...
function renderReputationLines(
  ...
  if (reputation.downloads.status === "ok") {
    ...
    if (reputation.downloads.trend === "spike") push(yellow(`⚠ download spike: ${reputation.downloads.trendDetail}`));
    else if (reputation.downloads.trend === "drop") push(yellow(`⚠ download drop: ${reputation.downloads.trendDetail}`));
    else if (weekly !== undefined && weekly < 100) push(yellow(`⚠ low adoption: ${weekly} weekly downloads`));
    else if (weekly !== undefined) push(dim(`✓ ~${weekly.toLocaleString("en-US")} weekly downloads`));
  } else if (reputation.downloads.status === "unavailable") {
    push(yellow("⚠ download stats unavailable — adoption UNKNOWN"));
  }
  if (reputation.repo.status === "ok" && reputation.repo.archived) push(yellow("⚠ GitHub repository is ARCHIVED"));
  else if (reputation.repo.status === "not-found") push(yellow("⚠ repository URL points to a missing/private GitHub repo"));
