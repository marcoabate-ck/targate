...
<footer>Click a node for details and its paths to the root · drag or scroll to pan, ⌘/Ctrl + scroll (or pinch) to zoom · filters keep the connecting paths so context is never lost · generated by targate (deterministic analysis, no AI)</footer>
...
(function () {
  ...
  // Esc deselects the current node (unless the search box has focus, where Esc
  // clears the query — let the input keep that native behavior).
  ...
  /* ---- pan & zoom (viewBox-based, cursor-anchored, constrained) ---- */
  var vb = svg.viewBox.baseVal;
  var viewport = document.getElementById("viewport");

  // The initial viewBox spans the whole drawing, so it IS the content extent.
  var CONTENT_W = vb.width, CONTENT_H = vb.height;
  var PAD = Math.max(40, Math.min(CONTENT_W, CONTENT_H) * 0.06); // breathing room past the blocks
  var MIN_W = Math.min(CONTENT_W, 400);                          // deepest zoom-in

  // Map a screen point to viewBox (user) coordinates, honoring the SVG's
  // preserveAspectRatio letterboxing — this is what anchors zoom exactly under
  // the cursor (a plain rect-ratio mapping drifts when the aspect ratios differ).
  function toWorld(clientX, clientY) {
    var ctm = svg.getScreenCTM();
    if (!ctm) return { x: vb.x + vb.width / 2, y: vb.y + vb.height / 2 };
    var p = svg.createSVGPoint();
    p.x = clientX; p.y = clientY;
    var w = p.matrixTransform(ctm.inverse());
    return { x: w.x, y: w.y };
  }

  // Keep the view inside the padded content box; if the view is larger than the
  // content on an axis (zoomed out past it), center that axis instead.
  function clampAxis(pos, size, content) {
    var min = -PAD, max = content + PAD, span = max - min;
    if (size >= span) return min - (size - span) / 2;
    return Math.min(max - size, Math.max(min, pos));
  }
  function clampView() {
    vb.x = clampAxis(vb.x, vb.width, CONTENT_W);
    vb.y = clampAxis(vb.y, vb.height, CONTENT_H);
  }
  ...
  viewport.addEventListener("wheel", function (ev) {
    ev.preventDefault();
    // Normalize the wheel delta across devices — line mode (classic mouse) and
    // page mode report small integers, pixel mode (trackpads / smooth-scroll
    // mice) reports many small events.
    var unit = ev.deltaMode === 1 ? 16 : ev.deltaMode === 2 ? viewport.clientHeight : 1;
    if (ev.ctrlKey || ev.metaKey) {
      // Ctrl/Cmd + wheel (and trackpad pinch, which the browser reports with
      // ctrlKey) zooms toward the cursor. Damp the per-event step, then clamp
      // the resulting scale so you can't zoom in past MIN_W or out past the
      // whole padded drawing.
      var factor = Math.exp(ev.deltaY * unit * 0.001);
      factor = Math.min(1.1, Math.max(1 / 1.1, factor));
      var maxFactor = Math.min((CONTENT_W + 2 * PAD) / vb.width, (CONTENT_H + 2 * PAD) / vb.height);
      var minFactor = MIN_W / vb.width;
      factor = Math.min(maxFactor, Math.max(minFactor, factor));
      var c = toWorld(ev.clientX, ev.clientY);
      vb.x = c.x - (c.x - vb.x) * factor;
      vb.y = c.y - (c.y - vb.y) * factor;
      vb.width *= factor; vb.height *= factor;
    } else {
      // Plain wheel/trackpad scroll pans the view. PAN_SPEED (<1) damps it so
      // momentum scrolling doesn't fling the graph across the viewport.
      var PAN_SPEED = 0.4;
      vb.x += ev.deltaX * unit * (vb.width / viewport.clientWidth) * PAN_SPEED;
      vb.y += ev.deltaY * unit * (vb.height / viewport.clientHeight) * PAN_SPEED;
    }
    clampView();
  }, { passive: false });
