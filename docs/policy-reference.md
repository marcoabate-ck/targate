...
| `requireApprovalForAdvisorySeverity`   | `"low"` \| `"moderate"` \| `"high"` \| `"critical"` | unset | A package whose worst known-vulnerability advisory (OSV) is **at or above** this severity → **require_approval**. The `ai-agent` preset sets `high`. Independent of any threshold, a **critical** advisory is `require_approval` by default (see note below). |
| `blockForAdvisorySeverity`             | `"low"` \| `"moderate"` \| `"high"` \| `"critical"` | unset | As above, but **block** at or above the level. If both thresholds match, block wins. Set `low` to disallow essentially any known vulnerability. |

When **either** advisory threshold is set, an advisory OSV could not grade
(`unknown` severity — no GHSA label, no numeric CVSS) **fails safe to
require_approval** rather than passing. With no threshold set, an ungraded
advisory does not escalate. Regardless of policy, a known **critical** advisory
is `require_approval` by default (not a hard block — a CVE is often unavoidable;
raise it to `block` with `blockForAdvisorySeverity: "critical"`).
