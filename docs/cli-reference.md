...
| `targate proxy <start\|stop\|status\|ensure\|setup\|teardown\|cert\|approvals\|approve\|deny>` | Run a registry proxy that vets every package before install. | `--port <n>`<br>`--upstream <url>`<br>`--host <addr>`<br>`--foreground`<br>`--tls`<br>`--dry-run` | `targate proxy setup`<br>`targate proxy status`<br>`targate proxy approve lodash@4.17.21` |
