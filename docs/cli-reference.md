...
| `targate proxy <start\|stop\|status\|ensure\|setup\|teardown\|cert>` | Run a registry proxy that vets every package before install (experimental). | `--port <n>`<br>`--upstream <url>`<br>`--host <addr>`<br>`--foreground`<br>`--tls` | `targate proxy setup`<br>`targate proxy status`<br>`targate proxy teardown` |
