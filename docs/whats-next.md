...
- [x] **Registry proxy** — `targate proxy`: a transparent, package-manager-agnostic enforcement point that vets every install before a tarball's bytes reach the machine, closing the gap left by CLI-only gating without a wrapper or lifecycle-script dependency (supported on npm/pnpm/yarn-berry — lockfile byte-identical with or without the proxy; yarn-classic/bun are refused since they would bake the proxy URL into the lockfile). Routing is via environment (`setup` writes a sourceable `~/.targate/proxy.env`, never the project `.npmrc`); private scopes are gated with `targate proxy exec`. Shipped: `setup`/`teardown`/`start`/`stop`/`status`/`ensure`/`exec`/`cert`, a local-CA HTTPS proxy that reuses the full deterministic pipeline (allow → serve, block → HTTP 403 with reason) and analyzes the exact bytes it will serve, an integrity-keyed persistent verdict cache (analyzed once, ever), per-scope private-registry uplinks (`setup` auto-captures private scopes from `.npmrc`, credentials 0600; the proxy authenticates upstream or relays pass-through), interactive out-of-band approval (a `require_approval` verdict holds the install while `targate proxy approve|deny` decides via a token-gated local control API; fail-closed on timeout), CA trust automation (`cert install`/`uninstall`: macOS login keychain, Windows per-user Root, Linux printed sudo step), a `doctor` check, and [docs](proxy.md).
...
## Phase 5 — Beyond the install gate

New user-facing capabilities that extend targate past the one-shot install decision — covering the *after* (drift over time), the *remedy*, and richer enforcement and reporting. Directional; each is a candidate for its own design doc before implementation.

- [ ] **Continuous monitoring + alerting** — a scheduled/daemon re-check of the *locked* dependency tree that fires a notification (Slack, generic webhook, or a GitHub issue) when a **new** advisory or malicious-package record lands for a version you have **already installed and approved**. The install-time gate is a snapshot; this closes the temporal gap — "you approved `lodash@x`; a critical CVE was just disclosed for it." Builds on `targate monitor` (which is one-shot today) by adding trigger cadence and outbound notification channels.
- [ ] **`targate fix` — risk-aware auto-remediation** — like `npm audit fix`, but risk-aware: given the advisories in your tree, compute the **minimal safe upgrade set** that clears them **without** introducing a version the pipeline would block or flag. Reuses `diff` (to rate each candidate upgrade) plus the deterministic gating and the new advisory-severity scoring. Turns targate from a gate into a gate **plus** a remediation planner; proposes the plan, then gates it like any other change.
- [ ] **Network-profile enforcement** — promote the sandbox's network *observation* into a declarative *allowlist*: record (or hand-write) the egress hosts a package is expected to contact during install, then **flag or block** an install whose lifecycle scripts reach an undeclared host ("the postinstall contacted `evil.com`"). A concrete anti-exfil / C2 signal at install time, built on the existing in-container DNS forwarder + logging proxy.
- [ ] **SBOM + VEX export** — `targate sbom`: emit a CycloneDX/SPDX bill of materials for the analyzed tree, annotated with targate's risk signals, plus VEX statements (which known CVEs are / are not exploitable in this tree). Reuses the existing analysis; unlocks integration with enterprise scanning and compliance workflows.
- [ ] **Publish anomaly detection** — a dedicated "release anomaly" report that composes existing reputation and behavior signals into an account-takeover heuristic: new maintainer **+** unusual version jump **+** newly added install script **+** lost provenance = high composite signal. Sharpens detection of the hardest case — a previously-trusted package compromised by a hijacked maintainer account.
