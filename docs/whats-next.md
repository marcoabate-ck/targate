...
- [x] **Registry proxy** *(experimental)* — `targate proxy`: a transparent, package-manager-agnostic enforcement point that vets every install before a tarball's bytes reach the machine, closing the gap left by CLI-only gating without a wrapper or lifecycle-script dependency (npm, pnpm, yarn, and bun all verified to route through it — npm's dist.tarball is left canonical, and for the other clients it is rewritten to a *clean* proxy URL, detected by user-agent, so pnpm/yarn-berry lockfiles stay portable; the real upstream URL is resolved server-side. yarn-classic/bun bake the absolute URL, a documented lockfile limitation). Shipped: `setup`/`teardown`/`start`/`stop`/`status`/`ensure`/`cert`, a local-CA HTTPS proxy that reuses the full deterministic pipeline (allow → serve, block → HTTP 403 with reason) and analyzes the exact bytes it will serve, an integrity-keyed persistent verdict cache (analyzed once, ever), per-scope private-registry uplinks (`setup` auto-migrates private scopes from `.npmrc`, capturing credentials 0600; the proxy authenticates upstream or relays pass-through), interactive out-of-band approval (a `require_approval` verdict holds the install while `targate proxy approve|deny` decides via a token-gated local control API; fail-closed on timeout), CA trust automation (`cert install`/`uninstall`: macOS login keychain, Windows per-user Root, Linux printed sudo step), a `doctor` check, and [docs](proxy.md). Design + PoC results in [docs/design/proxy.md](design/proxy.md).
