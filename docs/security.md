...
- **Tarball integrity is verified.** Downloaded tarballs are checked against the registry manifest's checksum (`dist.integrity` SRI, falling back to the legacy `dist.shasum`) before analysis; a mismatch aborts loudly. This ensures the bytes targate analyzes are exactly the bytes the registry (and your lockfile) pin — a substituting mirror/proxy cannot feed the analysis different content.
